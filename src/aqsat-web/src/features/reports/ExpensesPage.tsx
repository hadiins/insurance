...
export function ExpensesPage() {
  ...
  return (
    <div>
      ...
      <div className="mb-4.5 text-xs text-(--ice-3)">ثبت هزینه و گزارش به‌تفکیک دسته</div>
      ...
      <div className="mb-4.5 rounded-2xl border border-(--edge) bg-(--pane) p-5">
        <div className="mb-3.5 text-[13px] font-semibold text-(--ice)">ثبت هزینهٔ جدید</div>
        ...
        <>
          <div className="mb-4.5 grid grid-cols-2 gap-3">
            <div className="rounded-[14px] border border-(--edge) bg-(--pane) p-3.5">
              <div className="mb-1 text-[10px] tracking-[0.16em] text-(--ice-3)">جمع هزینه</div>
              <div className="text-[17px] font-extrabold tracking-tight text-(--ember)">{money(report.totalAmount)}</div>
            </div>
            <div className="rounded-[14px] border border-(--edge) bg-(--pane) p-3.5">
              <div className="mb-1 text-[10px] tracking-[0.16em] text-(--ice-3)">تعداد هزینه</div>
            ...
            <table className="w-full border-collapse">
              <tbody>
                {report.byCategory.map((c) => (
                  <tr key={c.categoryName} className="border-t border-(--edge) first:border-t-0">
                    <td className="px-3 py-2.5 text-[13px] text-(--ice-3)">{c.categoryName}</td>
                    <td className="px-3 py-2.5 text-[12px] text-(--ice-3)">{fa(c.count)} مورد</td>
              ...
              <tbody>
                {report.rows.map((r) => (
                  <tr key={r.id} className="border-t border-(--edge) first:border-t-0">
                    ...
                    <td className="px-3 py-2.5 text-[13px] font-semibold">{r.title}</td>
                    <td className="px-3 py-2.5 text-[13px] text-(--ice-3)">{r.categoryName}</td>
                    <td className="px-3 py-2.5 text-[13px] font-bold">{money(r.amount)}</td>
                    <td className="px-3 py-2.5 text-[12px] text-(--ice-3)">{r.methodType === "Cash" ? "نقدی" : "واریز بانکی"}</td>
