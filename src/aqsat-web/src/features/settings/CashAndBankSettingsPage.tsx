  ...
  return (
    <div>
      ...
      <div className="mb-4.5 overflow-hidden rounded-2xl border border-(--edge) bg-(--pane)">
            ...
            <tbody>
              {boxes?.map((b) => (
                <tr key={b.id} className="border-t border-(--edge) first:border-t-0">
                  <td className="px-3 py-2.5 text-[13.5px] font-semibold">{b.name}</td>
                  ...
                  <td className="px-3 py-2.5 text-[13.5px]">
                    <StatusBadge tone={b.isActive ? "mint" : "neutral"}>{b.isActive ? "فعال" : "غیرفعال"}</StatusBadge>
                  </td>
                  <td className="px-3 py-2.5 text-[13.5px]">
                    <button type="button" onClick={() => toggleBox(b)} className="ms-2 text-[11.5px] text-(--ice-3) hover:text-(--ice)">
                      {b.isActive ? "غیرفعال کردن" : "فعال کردن"}
                    </button>
                    <button type="button" onClick={() => removeBox(b.id)} className="ms-2 text-[11.5px] text-(--ember) hover:brightness-110">
                      حذف
                    </button>
                  </td>
                </tr>
        ...
        <div className="flex items-end gap-2 border-t border-(--edge) p-3">
          <div className="flex-1">
            <label className="mb-1.5 block text-[11.5px] tracking-wider text-(--ice-3)">نام صندوق</label>
            <input value={newBoxName} onChange={(e) => setNewBoxName(e.target.value)} className={inputClass} />
          </div>
          <div className="w-44">
            <label className="mb-1.5 block text-[11.5px] tracking-wider text-(--ice-3)">ماندهٔ ابتدای دوره</label>
      ...
      <div className="overflow-hidden rounded-2xl border border-(--edge) bg-(--pane)">
            ...
            <tbody>
              {accounts?.map((a) => (
                <tr key={a.id} className="border-t border-(--edge) first:border-t-0">
                  <td className="px-3 py-2.5 text-[13.5px] font-semibold">{a.bankName}</td>
                  <td className="px-3 py-2.5 text-[13.5px] tabular-nums" dir="ltr">
                    {a.accountNumber}
                  </td>
                  <td className="px-3 py-2.5 text-[13.5px] text-(--ice-3)">{a.accountHolderName ?? "—"}</td>
                  <td className="px-3 py-2.5 text-[13.5px]">
                    <OpeningBalanceInput
                  ...
                  <td className="px-3 py-2.5 text-[13.5px]">
                    <StatusBadge tone={a.isActive ? "mint" : "neutral"}>{a.isActive ? "فعال" : "غیرفعال"}</StatusBadge>
                  </td>
                  <td className="px-3 py-2.5 text-[13.5px]">
                    <button type="button" onClick={() => toggleAccount(a)} className="ms-2 text-[11.5px] text-(--ice-3) hover:text-(--ice)">
                      {a.isActive ? "غیرفعال کردن" : "فعال کردن"}
                    </button>
                    <button type="button" onClick={() => removeAccount(a.id)} className="ms-2 text-[11.5px] text-(--ember) hover:brightness-110">
                      حذف
                    </button>
                  </td>
                </tr>
        ...
        <div className="flex items-end gap-2 border-t border-(--edge) p-3">
          <div className="w-40">
            <label className="mb-1.5 block text-[11.5px] tracking-wider text-(--ice-3)">نام بانک</label>
          ...
          <div className="w-40">
            <label className="mb-1.5 block text-[11.5px] tracking-wider text-(--ice-3)">شمارهٔ حساب</label>
            <input value={newAccountNumber} onChange={(e) => setNewAccountNumber(e.target.value)} dir="ltr" className={inputClass} />
          </div>
          <div className="flex-1">
            <label className="mb-1.5 block text-[11.5px] tracking-wider text-(--ice-3)">صاحب حساب (اختیاری)</label>
      ...
      <div className="mt-4.5 overflow-hidden rounded-2xl border border-(--edge) bg-(--pane)">
            ...
            <tbody>
              {banks?.map((b) => (
                <tr key={b.id} className="border-t border-(--edge) first:border-t-0">
                  ...
                  <td className="px-3 py-2.5 text-[13.5px]">
                    <button type="button" onClick={() => toggleBank(b)} className="ms-2 text-[11.5px] text-(--ice-3) hover:text-(--ice)">
                      {b.isActive ? "غیرفعال کردن" : "فعال کردن"}
                    </button>
                    <button type="button" onClick={() => removeBank(b.id)} className="ms-2 text-[11.5px] text-(--ember) hover:brightness-110">
                      حذف
                    </button>
                  </td>
                </tr>
        ...
        <div className="flex items-end gap-2 border-t border-(--edge) p-3">
          <div className="flex-1">
            <label className="mb-1.5 block text-[11.5px] tracking-wider text-(--ice-3)">نام بانک (اگر در فهرست نیست)</label>
