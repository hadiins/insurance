...
/// default write-off + operating expense) = سود خالص. The five inputs are aggregated by the caller
/// (real DB queries); this is only the arithmetic guard — income/expense/net must never silently
/// drift apart from their five parts, which is exactly what "hand-compute one month and match it to
/// the rial" checks. OperatingExpense (stage 7/7) is the agency's own real costs (Expense), never a
/// derived/estimated figure like default write-off.
/// </summary>
